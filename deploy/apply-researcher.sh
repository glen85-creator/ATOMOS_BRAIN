#!/usr/bin/env bash
# E층 β-fanout — RESEARCHER 슬롯만 타깃 활성화 PATCH (apply-marketing.sh 미러)
# VPS 호스트에서 실행. 사전: git pull (payloads/ATOMOS_RESEARCHER.json 최신)
set -euo pipefail
cd "$(dirname "$0")"
source ./uuid-map.env

PC="${PC:-paperclip-sab7-paperclip-1}"
PAYLOAD="payloads/ATOMOS_RESEARCHER.json"
...
echo "== 2) RESEARCHER PATCH ($ATOMOS_RESEARCHER) =="
RESP=$(docker exec -i -u node "$PC" bash -lc "curl -s -b /tmp/pc.cookies -X PATCH 'http://localhost:3100/api/agents/$ATOMOS_RESEARCHER' -H 'Content-Type: application/json' -H 'Origin: http://localhost:3100' --data-binary @-" < "$PAYLOAD")
