...
LLM이 직접 위키를 **컴파일**해 유지보수하는 지식 관리 패턴. 안드레이 카파시가 2026-04에 제안.

## 핵심 아이디어

- **RAG와의 차이**: RAG는 매 질의마다 원본을 검색·재합성한다. LLM Wiki는 LLM이 사전에 원본을 읽어 위키 페이지로 "컴파일"해 두고, 질의 시에는 위키를 참조한다. 카파시의 비유: 소스코드(raw) → 실행 파일(wiki) 컴파일.
- **3-레이어 구조**:
...
- **카파시 본인**: 논문·연구 자료 정리
- **Jay Choi**: "나 자신"을 주제로 — 개인 서비스 이력·실패·인사이트를 위키화
