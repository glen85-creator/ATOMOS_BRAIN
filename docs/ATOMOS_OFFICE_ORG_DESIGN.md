...
| `TOOLS.md` | ① 허용 toolsets (v1: `terminal`만) ② terminal 플레이북 — fanout 슬롯=3단계(이슈 fetch → 분석/산출 → 결과 JSON 을 /tmp 저장 후 python3 urllib 로 코멘트+done 게시; 멀티라인 인라인 이스케이프 회피), CEO 슬롯=인라인 curl 4단계 유지 ③ 도구별 호출 예시 ④ Pack 자격증명은 환경변수 참조만 | 중간 |
...
- TOOLS: terminal만. 플레이북(3단계): `curl -s {{paperclipApiUrl}}/issues/{{taskId}}` → 분석/산출 → 결과 JSON 을 /tmp 저장 후 `python3` urllib 로 코멘트+done 게시. **3단계 외 명령 금지** (SOUL 금지 조항과 이중 잠금). CEO 슬롯만 예외적으로 인라인 curl 4단계.
...
| terminal+`--yolo` | 승인된 트레이드오프 (✅ S1). 잠금 3중: ①SOUL 금지조항 ②TOOLS 플레이북 한정(fanout 3단계 / CEO 4단계) ③작업 후 재-pause. blast 반경 = Paperclip 컨테이너 한정 |
